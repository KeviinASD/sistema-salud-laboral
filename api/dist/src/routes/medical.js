...
router.post("/history", async (req, res) => {
    try {
        const { admision_id, motivo_consulta, anamnesis, examen_fisico, diagnostico, tratamiento, observaciones } = req.body;
        ...
        if (historiaExistente) {
            ...
            const historia = await prisma.historiaClinica.update({
                where: { admision_id },
                data: {
                    motivo_consulta: motivo_consulta || historiaExistente.motivo_consulta,
                    anamnesis: anamnesis || historiaExistente.anamnesis,
                    ...
                    observaciones: observaciones || historiaExistente.observaciones,
...
router.put("/clinical-history/:id", async (req, res) => {
    try {
        const { anamnesis, examen_fisico, diagnostico, tratamiento, observaciones } = req.body;
        const updateData = {
            updated_by: req.user?.id
        };
        if (anamnesis !== undefined)
            updateData.anamnesis = anamnesis;
        if (examen_fisico !== undefined)
            updateData.examen_fisico = examen_fisico;
...
router.post("/clinical-history/:id/progress-notes", async (req, res) => {
    try {
...
router.post("/clinical-history/:id/attachments", upload.single("file"), async (req, res) => {
    try {
        const { tipo } = req.body;
        const file = req.file;
        if (!file || !tipo) {
...
router.put("/exams/:type/:id", async (req, res) => {
    try {
...
router.post("/vitals/record", async (req, res) => {
    try {
        ...
        const signosActuales = historia.examen_fisico?.signos_vitales || [];
        signosActuales.push({
            ...signos_vitales,
            fecha: new Date(),
